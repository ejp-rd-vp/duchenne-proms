...
#puts "comorbidity_clauses.rdf", promlist.class
#puts "comorbidity_clauses.rdf", promlist.first
...
promlist.sort.each do |prom|
  ...
  split = data.split('||')
  next unless split[0] == "Problems and comorbidity"
...
ordered.each do |question|
  ...
  clause = %{
    ...
    puts "comorbidity_clauses.rdf", clause
    questions = questions + %{<rdf:Description rdf:about="https://w3id.org/duchenne-proms#DDP_#{number}"/>\n}
    count +=1
      
end

puts "comorbidity_clauses.rdf", %{
    <owl:Class rdf:about="https://w3id.org/duchenne-proms#problems_and_comorbidity_section">
        <owl:equivalentClass>
            <owl:Restriction>
...
#{questions}
        ...
        <rdfs:subClassOf rdf:resource="http://semanticscience.org/resource/SIO_000171"/>
               <rdfs:label rdf:datatype="http://www.w3.org/2001/XMLSchema#string">Comorbidity Section</rdfs:label>
